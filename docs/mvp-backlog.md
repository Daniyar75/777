...
**Implementation status (this repo):** BL-201, BL-202, BL-204, BL-205, BL-207 are implemented — see `services-or-modules/relationship-crm` (Contact CRUD, multi-role, normalized-field duplicate detection, consent lifecycle, unified timeline), `services-or-modules/work-management` (Task create/read/list/complete/delegate), and `apps/api/src/routes/{contacts,tasks}.ts`. Object-level ABAC (owned/assigned scope against a real row, not just tenant-wide admin actions) now lives in the shared `packages/access` (promoted out of relationship-crm once work-management needed the identical shape). `apps/web` (ADR-0011) now provides a working UI over all of this: login/MFA/tenant-switch, Contacts (create with duplicate-candidate review, roles, consent, activity, unified timeline), and Tasks (create/complete) — walked through end to end in a real browser against the real API, screens matching `docs/ui/information-architecture.md` UI-001/003/004/005/013. Known gaps: BL-202's duplicate detection is a direct normalized-field DB lookup, not the dedicated search index ADR-0007 describes (fine at MVP scale, revisit if lookup latency becomes an issue); BL-203 (import/export) and BL-206 (merge resolution — duplicates surface but are never merged) are not started; BL-207's calendar view, Event/EventParticipant, and task delegation UI are not started (Task create/complete itself is, both backend and UI); Communication (FR-COMM) is deferred, so the timeline currently merges Activity/Consent/ContactRole only, not messages.

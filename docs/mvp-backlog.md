...
**Implementation status (this repo):** BL-201, BL-202, BL-203, BL-204, BL-205, BL-206, BL-207 are implemented — see `services-or-modules/relationship-crm` (Contact CRUD, multi-role, normalized-field duplicate detection, consent lifecycle, unified timeline, CSV import with client-side column mapping/dry-run/commit/safe-rerun, CSV export, merge with survivor/duplicate resolution via `merged_into_id` aliasing), `services-or-modules/work-management` (Task create/read/list/complete/delegate), and `apps/api/src/routes/{contacts,tasks,tenants}.ts` (including `GET /memberships` for assignee pickers). Object-level ABAC (owned/assigned scope against a real row, not just tenant-wide admin actions) lives in the shared `packages/access` (promoted out of relationship-crm once work-management needed the identical shape). `apps/web` (ADR-0011) now provides a working UI over all of this: login/MFA/tenant-switch, Contacts (create with duplicate-candidate review, roles, consent, activity, unified timeline, CSV import screen, CSV export button, per-contact duplicate list with one-click merge), and Tasks (create/complete/delegate with an assignee picker backed by `GET /memberships`) — walked through end to end in a real browser against the real API, screens matching `docs/ui/information-architecture.md` UI-001/003/004/005/013. Known gaps: BL-202's duplicate detection is a direct normalized-field DB lookup, not the dedicated search index ADR-0007 describes (fine at MVP scale, revisit if lookup latency becomes an issue); BL-206's merge UI does not yet expose the field-resolution conflict picker (`MergeContactsRequest.field_resolutions` is supported server-side and covered by tests, but the UI always keeps the survivor's own field values — a future refinement once real conflicting-data merges are common); BL-207's calendar view and Event/EventParticipant are not started (Task create/complete/delegate itself is, both backend and UI); Communication (FR-COMM) is deferred, so the timeline currently merges Activity/Consent/ContactRole only, not messages.
